  ...
  <htmlpageheader name="page-header">
    <div class="row mb-3 print-header">
      ...
      {{-- <div class="col-md-6 text-right" style="width: 50%;float:left;padding-top: 50px">
        ...
        <img src="{{ str_replace('/assets','assets', $purchaseOrder->Unit->hr_unit_logo) }}" alt="logo"
      ...
      <tbody>
        ...
        {{-- <tr>
          <td colspan="2" style="border: none !important">
            <small>(Note: This {{ $directPurchase ? 'Internal Job Order' : 'Purchase Order' }} doesn’t require signature
              as it is automatically generated from MBM Group’s ERP)</small>
          </td>
        </tr> --}}
        {{-- <tr>
              ...
              @if(isset($searchAccounts[0]))
                @foreach($searchAccounts as $searchAccount)
                  <li>{{ '[' . $searchAccount->code . '] ' . $searchAccount->name }}
                    ({{ '[' . $searchAccount->accountGroup->code . '] ' . $searchAccount->accountGroup->name }})</li>
                @endforeach
              @endif
            </ul>
          </td>
