	...
	$(document).ready(function() {
		...
		var columns = [{
            ...
            render: function(data, type, row, meta) {
                return '<p style="width: 50px;white-space: normal">'+(meta.row + meta.settings._iDisplayStart + 1)+'</p>';
            }
        }];
		...
		var table = $('.datatable-serverside').DataTable({
            ...
            language: {
              ...
              "processing": "<i class='las la-spinner la-spin' style='font-weight: bold;font-size:18px'></i>&nbsp;&nbsp;Processing, Please wait..."
          	},
            ...
            initComplete: function (settings, json) {
                ...
                var checkAll = "{{ isset($checkAll) ? $checkAll : false }}";
                var table = this.api();
                var count = 0;
                table.columns().every(function () {
                    ...
                    if(checkAll && count == checkAll){
                        var input = $('<center><input class="form-check-input check-all-checkboxes" type="checkbox" name="chkbx_all_first" id="check-all-checkboxes" onclick="CheckAllCheckboxes()" checked style="transform: scale(1.5, 1.5);margin-top: -10px !important">'+
                        '<label class="form-check-label mt-8" for="check-all-checkboxes"></label></center>')
                        .appendTo($(column.header())
                        .empty());
                    }else{
                        var input = $('<input type="text" style="width: '+(count == 1 ? '50px' : '100%')+' !important;" placeholder="'+header+'" title="'+header+'"/><span style="display: none">'+header+'</span>')
                        .appendTo($(column.header())
                        .empty());
                    }
